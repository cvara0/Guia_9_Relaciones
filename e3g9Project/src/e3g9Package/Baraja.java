...
public class Baraja {
    ...
    public void crearBarajaEspaniola() {
        this.nuevaBaraja = new ArrayList<>();
        for (byte i = 1; i <= 12; i++) {
            if (i < 8 || i > 9) {
                this.nuevaBaraja.add(new Carta(i, 'e'));
                this.nuevaBaraja.add(new Carta(i, 'b'));
                this.nuevaBaraja.add(new Carta(i, 'o'));
